"""Conftest for social-api — uses FastAPI dependency_overrides to stub JWT auth.
...
def app_module():
    for k in ("ws", "main"):
        if k in sys.modules:
            del sys.modules[k]
    spec = importlib.util.spec_from_file_location("social_main", os.path.join(ROOT, "main.py"))
