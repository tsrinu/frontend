...
**Port:** `8013` · **Module:** `main:app`

Live streams, polls, predictions, WebSocket chat

## Run locally

```cmd
cd C:\Users\slua_187012ca5b4f\Documents\API\live-api
...
Then open: **http://127.0.0.1:8013/docs** — interactive Swagger UI.

## Run in Docker

```cmd
docker build -t distrebute-live-api .
docker run -p 8013:8013 -e DEV_MODE=true distrebute-live-api
```
...
| Method | Path | Returns |
|---|---|---|
| GET | `/healthz` | `{status: "ok", service: "live-api"}` |
...
| Var | Default | Notes |
|---|---|---|
| `PORT` | 8013 | Override only if your reverse proxy needs a different port |
